...
def neo_hookean(
    ...
    mu = jnp.asarray(cell_data["mu"])
    lambda_ = jnp.asarray(cell_data["lambda"])
    F = tetra.deformation_gradient(disp, points)
    J = jnp.linalg.det(F)
    C = F.T @ F
    I_C = jnp.trace(C)
    # logJ = jnp.where(J > 1e-6, jnp.log(J), J - 1)
    # W = 0.5 * mu * (I_C - 3) - mu * logJ + 0.5 * lambda_ * logJ**2
    W = 0.5 * mu * (I_C - 3) + 0.5 * lambda_ * (J - 1) ** 2
    return W
