...
mesh: pv.UnstructuredGrid = mkit.ext.tetwild_wrapped(outer, inner, coarsen=True)
ic(mesh)
...
centers: pv.PolyData = mesh.cell_centers(progress_bar=True)
ic(centers)
mesh.cell_data["mu"] += 3 * cfg.mu * centers.points[:, 2]
mesh.cell_data["lambda"] += 3 * cfg.lambda_ * centers.points[:, 2]
