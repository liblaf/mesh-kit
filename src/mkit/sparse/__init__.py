...
from mkit.sparse.typing import AnySparseArray


def sparse_mask(
    _array: AnySparseArray, _coord_masks: Sequence[npt.ArrayLike], /
) -> sparse.COO:
    array: sparse.COO = sparse.as_coo(_array)
    coord_masks: list[npt.NDArray[np.bool]] = [
        np.asarray(mask, bool) for mask in _coord_masks
    ]
    ...
    return sparse.COO(new_coords, array.data[data_mask], shape=new_shape)
